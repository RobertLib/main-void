  ...
  for (int y = 0; y < MAP_HEIGHT; y++)
  {
    for (int x = 0; x < MAP_WIDTH; x++)
    {
      ...
      for (int i = 0; i < TILE_COUNT; i++)
      {
        ...
        {
          Rectangle destRect = {
              x * TILE_SIZE * SCALE + offset.x,
              y * TILE_SIZE * SCALE + offset.y,
...
void updatePlayer(float dt)
{
  ...
  if (IsKeyPressed(KEY_UP) && (player.isOnGround || aboveLadder))
    player.vel.y = -player.speed / 3;

  bool onKey = GET_TILE((int)centerX / TILE_SIZE, (int)bottomHalfY / TILE_SIZE) == 'K';
...
{
  Rectangle destRect = {
      player.pos.x * SCALE + offset.x,
      player.pos.y * SCALE + offset.y,
...
void updateOffset()
{
  int screenWidth = IsWindowFullscreen() ? GetMonitorWidth(0) : GetScreenWidth();
  int screenHeight = IsWindowFullscreen() ? GetMonitorHeight(0) : GetScreenHeight();
  offset.x = (screenWidth - MAP_WIDTH * TILE_SIZE * SCALE) / 2;
  offset.y = (screenHeight - MAP_HEIGHT * TILE_SIZE * SCALE) / 2;
}
...
{
  int tileX = (mouseX - offset.x) / (TILE_SIZE * SCALE);
  int tileY = (mouseY - offset.y) / (TILE_SIZE * SCALE);
